...
echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') dero is trading at $get_usdt_dero_quote"
...
echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') the dero amount $formatted is trading for $amount_usdt USDT"
...
echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') xmr is trading at $get_usdt_xmr_bid USDT"

# Calculate amount in XMR based on the calculated amount in USDT
amount_xmr=$(bc <<< "scale=12;$amount_usdt / $get_usdt_xmr_bid")
echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') the dero amount $formatted is worth $amount_xmr xmr"

# Calculate a % of XMR as fee
fee_xmr=$(echo "$amount_xmr * 0.01" | bc)
echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') the xmr $amount_xmr fee is $fee_xmr xmr"

# Calculate a % of XMR for the trade after fee deduction
trade_xmr=$(echo "$amount_xmr * 0.99" | bc)
echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') the xmr $amount_xmr to be sent is $trade_xmr"

# Remove decimals and leading zeroes
amnt=$(echo "$trade_xmr" | awk '{ gsub(/^0+|\./,""); print}')

echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') send XMR to recipient | amount $amnt"
