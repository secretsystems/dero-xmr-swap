...
echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') xmr is trading at $get_usdt_xmr_quote"
...
echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') the xmr amount $formatted is trading at $amount_usdt"
...
echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') dero is trading at $get_usdt_dero_ask"

# Calculate amount in DERO based on the calculated amount in USDT
amount_dero=$(bc <<< "scale=5;$amount_usdt / $get_usdt_dero_ask")
echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') the xmr amount $formatted is worth at $amount_dero"

# Calculate a % of DERO as fee
fee_dero=$(echo "$amount_dero * 0.01" | bc)
echo "SERVICE MSG: $(date '+%Y-%m-%d %H:%M:%S') the dero $amount_dero fee is $fee_dero"
